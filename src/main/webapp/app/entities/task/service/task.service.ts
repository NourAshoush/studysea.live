...
type RestOf<T extends ITask | NewTask> = Omit<T, 'start' | 'creation'> & {
  start?: string | null;
...
export class TaskService {
  ...
  getByUser(id: number | undefined): Observable<ITask[]> {
    // eslint-disable-next-line no-console,@typescript-eslint/restrict-template-expressions
    console.log('task.service.ts', this.http.get<ITask[]>(`${this.resourceUrl}/getTasksList/${id}`));
    // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
    return this.http.get<ITask[]>(`${this.resourceUrl}/getTasksList/${id}`);
  }

  protected convertDateFromClient<T extends ITask | NewTask | PartialUpdateTask>(task: T): RestOf<T> {
    return {
      ...task,
      start: task.start?.toJSON() ?? null,
      creation: task.creation?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restTask: RestTask): ITask {
    return {
      ...restTask,
      start: restTask.start ? dayjs(restTask.start) : undefined,
      creation: restTask.creation ? dayjs(restTask.creation) : undefined,
    };
  }
