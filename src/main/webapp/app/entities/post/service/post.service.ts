...
type RestOf<T extends IPost | NewPost> = Omit<T, 'creationTime'> & {
...
export class PostService {
  ...
  protected convertDateFromClient<T extends IPost | NewPost | PartialUpdatePost>(post: T): RestOf<T> {
    return {
      ...post,
      creationTime: post.creationTime?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPost: RestPost): IPost {
    return {
      ...restPost,
      creationTime: restPost.creationTime ? dayjs(restPost.creationTime) : undefined,
    };
  }
