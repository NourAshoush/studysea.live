...
type RestOf<T extends IComment | NewComment> = Omit<T, 'creationTime'> & {
...
export class CommentService {
  ...
  protected convertDateFromClient<T extends IComment | NewComment | PartialUpdateComment>(comment: T): RestOf<T> {
    return {
      ...comment,
      creationTime: comment.creationTime?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restComment: RestComment): IComment {
    return {
      ...restComment,
      creationTime: restComment.creationTime ? dayjs(restComment.creationTime) : undefined,
    };
  }
