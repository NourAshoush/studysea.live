...
type RestOf<T extends IReport | NewReport> = Omit<T, 'reportedTime'> & {
...
export class ReportService {
  ...
  protected convertDateFromClient<T extends IReport | NewReport | PartialUpdateReport>(report: T): RestOf<T> {
    return {
      ...report,
      reportedTime: report.reportedTime?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restReport: RestReport): IReport {
    return {
      ...restReport,
      reportedTime: restReport.reportedTime ? dayjs(restReport.reportedTime) : undefined,
    };
  }
